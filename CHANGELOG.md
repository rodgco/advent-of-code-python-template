...
This version documents enhancements and features that were added in the enhanced version by rodgco.

### New Features

- :sparkles: add `--no-submit` flag to prevent automatic answer submission when running with real input
- :sparkles: add `--spinner` flag to display a spinner animation during long-running operations for visual feedback on progress
- :sparkles: add `IntSplit2ColSolution` class to parse 2D integer input with columns (previously had only row-based support)
- :sparkles: add `AnyFunSolution` class for custom parsing logic via the `fun()` method
- :sparkles: add `StrMatchRegexpSolution` class for parsing lines using regular expressions
- :sparkles: improve `spinner()` method with configurable update frequency via `count` parameter
- :sparkles: enhance README with comprehensive documentation of all available base classes and their usage
...
- improved error messages for missing or empty input files
- enhance `BaseSolution` with better generic typing support
- add comprehensive documentation of spinner functionality with examples
- improve documentation for custom input parsing strategies

### Internal Updates

- update example solution template to use `StrSplitSolution` as default (changed from `TextSolution`)
- add `show_spinner` parameter to solution initialization
- refine code formatting and documentation
